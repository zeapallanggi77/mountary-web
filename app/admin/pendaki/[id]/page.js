...
export default function DetailPendaki() {
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    if (pendaki) {
      console.log("DATA DETAIL:", pendaki);
      console.log("FOTO PROFIL:", pendaki.foto_identitas);
    }
  }, [pendaki]);
  ...
  if (!pendaki) return (
    <div className="min-h-screen bg-[#F6F0D7] flex flex-col items-center justify-center font-black">
      ...
      <button onClick={() => router.back()} className="mt-4 bg-[#3D4F3E] text-white px-6 py-2 rounded-xl text-xs hover:bg-black transition text-uppercase">KEMBALI</button>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#F6F0D7] font-sans text-slate-900">
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-12 mt-4 md:mt-8">
        <div className="flex items-center justify-between mb-12">
          <button 
            onClick={() => router.back()}
            className="bg-[#FCF9F0] border-2 border-[#E5DEC1] px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-[#3D4F3E] hover:text-white transition shadow-sm flex items-center gap-2"
          >
            <span className="text-lg">−</span> KEMBALI
          </button>
          
          <div className="text-right">
            <div className="inline-block bg-[#3D4F3E] text-[#F6F0D7] px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-tighter mb-2 shadow-sm">
              ADMIN PANEL
            </div>
            <h1 className="text-3xl font-black uppercase tracking-tighter text-[#3D4F3E] leading-none">PROFIL PENDAKI</h1>
<button
  onClick={() => {
    navigator.clipboard.writeText(pendaki.kode_pendaki);
    setCopied(true);
    setTimeout(() => setCopied(false), 1500);
  }}
  className="flex items-center gap-2 text-[10px] text-[#A6915C] font-bold uppercase italic tracking-[0.2em] mt-2 hover:underline"
>
  <span>{copied ? "✔" : "📋"}</span>
          ...
          <div className="space-y-8">
            <div className="bg-[#FCF9F0] p-10 rounded-[48px] shadow-xl shadow-[#3d4f3e]/5 border border-[#E5DEC1] text-center relative overflow-hidden">
              ...
              <div className="w-40 h-40 bg-white rounded-full mx-auto mb-8 overflow-hidden border-8 border-[#F6F0D7] shadow-inner flex items-center justify-center relative">
                {/* PROFIL BUNDAR: NARIK DARI foto_ktp */}
                {pendaki.foto_identitas ? (
                  <img 
                    src={`/uploads/${pendaki.foto_identitas}`}
                    alt="Avatar" 
                    className="w-full h-full object-cover profile-img" 
                    onError={(e) => {
                      e.target.style.display='none';
                      e.target.parentElement.querySelector('.fallback-user').style.display='block';
                    }} 
                  />
                ) : null}
                <span className={`fallback-user text-7xl ${pendaki.foto_ktp ? 'hidden' : 'block'}`}>👤</span>
              </div>
          ...
          <div className="lg:col-span-2 space-y-10">
            <div className="bg-[#FCF9F0] p-10 rounded-[48px] border border-[#E5DEC1] shadow-sm">
              <div className="flex items-center gap-4 mb-10">
                  <div className="w-10 h-10 bg-[#3D4F3E] text-[#F6F0D7] rounded-2xl flex items-center justify-center font-black shadow-md italic">ID</div>
...
{
  label: 'Tanggal Lahir',
  val: pendaki.tgl_lahir
    ? new Date(pendaki.tgl_lahir).toLocaleDateString("id-ID", {
        day: "2-digit",
        month: "long",
        year: "numeric",
      })
    : "-"
            ...
            <div className="bg-[#FCF9F0] p-10 rounded-[48px] border border-[#E5DEC1] shadow-sm">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                {[
                  {label: 'PROVINSI', val: pendaki.provinsi},
                  {label: 'KOTA', val: pendaki.kota},
                  {label: 'KECAMATAN', val: pendaki.kecamatan},
                  {label: 'KELURAHAN', val: pendaki.kelurahan},
                ].map((item, i) => (
                  <div key={i} className="bg-[#F6F0D7] p-5 rounded-[28px] border border-[#E5DEC1] shadow-inner">
                    <p className="text-[8px] font-black text-[#A6915C] uppercase mb-1">{item.label}</p>
                    <p className="text-[11px] font-black uppercase text-[#3D4F3E] leading-tight">{item.val || "-"}</p>
                  </div>
                ))}
              </div>
              <div className="bg-white/40 p-6 rounded-3xl border border-[#E5DEC1] border-dashed">
                <p className="text-[8px] font-black text-[#A6915C] uppercase mb-2 tracking-[0.2em]">ALAMAT LENGKAP</p>
                <p className="text-sm font-bold italic leading-relaxed text-[#3D4F3E]">{pendaki.alamat || 'Alamat tidak diisi'}</p>
              </div>
            </div>

            <div className="bg-[#3D4F3E] p-10 rounded-[48px] text-[#F6F0D7] shadow-2xl relative overflow-hidden">
              <h3 className="text-xs font-black uppercase tracking-[0.3em] mb-10 text-[#A6915C] italic">KONTAK DARURAT</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                <div className="bg-white/5 p-8 rounded-[32px] border border-white/10">
                  <p className="text-[9px] font-bold opacity-60 uppercase mb-2 tracking-widest">NO. TELEPON</p>
                  <p className="text-xl font-black tracking-[0.2em]">{pendaki.no_telp || "-"}</p>
                </div>
                <div className="bg-[#BC6C25]/20 p-8 rounded-[32px] border border-[#BC6C25]/40">
                  <p className="text-[9px] font-bold text-[#BC6C25] uppercase mb-2 tracking-widest">NO. DARURAT</p>
                  <p className="text-xl font-black tracking-[0.2em] text-white">{pendaki.no_darurat || "-"}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#FCF9F0] p-10 rounded-[48px] border border-[#E5DEC1] shadow-sm">
              ...
              <div className="relative group">
                 {/* KOTAK LAMPIRAN: NARIK DARI foto_identitas */}
                 {pendaki.foto_ktp ? (
                    <div className="overflow-hidden rounded-[40px] border-[12px] border-white shadow-2xl transition-transform duration-500 hover:scale-[1.01]">
                      <img src={`/uploads/${pendaki.foto_ktp}`}
                        alt="Lampiran Identitas" 
                        className="w-full h-auto grayscale-[0.2] hover:grayscale-0 transition-all duration-700"
                        onError={(e) => {
                          e.target.parentElement.innerHTML = `<div class="p-20 text-center font-black opacity-30 uppercase italic text-[#A6915C]">File ${pendaki.foto_identitas} Not Found</div>`;
                        }}
                      />
                    </div>
                 ) : (
                    <div className="h-72 bg-[#F6F0D7] rounded-[40px] border-4 border-dashed border-[#E5DEC1] flex flex-col items-center justify-center text-[#A6915C]">
                       ...
                       <p className="text-xs font-black uppercase tracking-[0.3em] opacity-60 italic text-center leading-relaxed">IDENTITAS BELUM<br/>TERUNGGAH</p>
                    </div>
                 )}
              </div>
            </div>
            ...
            <div className="pt-10">
              <button 
                ...
                className={`w-full py-8 rounded-[32px] font-black uppercase text-xs tracking-[0.4em] transition-all duration-300 shadow-xl border-b-8 border-black/20 active:border-b-0 active:translate-y-1 flex items-center justify-center gap-4 ${
                  pendaki.status === 'suspended' 
                  ? "bg-[#3D4F3E] text-[#F6F0D7] hover:bg-black" 
                  : "bg-[#7B341E] text-[#F6F0D7] hover:bg-black" 
                }`}
              >
